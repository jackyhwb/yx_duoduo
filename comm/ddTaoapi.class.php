...
class ddTaoapi extends Taoapi{
	...
	function taobao_tbk_item_get($data){
		...
		if(!isset($data['fields']) || $data['fields']==''){
		    $data['fields'] = 'num_iid,title,pict_url,small_images,reserve_price,zk_final_price,user_type,provcity,item_url';
		}
		...
		if(isset($data['is_tmall'])){
		    $this->is_tmall = $data['is_tmall'];
		}
		...
		if(isset($TaobaokeData['code'])){
			...
			return array('s'=>1,'r'=>$TaobaokeData['results']['n_tbk_item'],'total'=>$TaobaokeData['total_results']);
		}
	}
	
	function taobao_tbk_item_info_get($num_iids){
		$this->method = 'taobao.tbk.item.info.get';
		$this->fields='num_iid,title,pict_url,small_images,reserve_price,zk_final_price,user_type,provcity,item_url';
	...
	function taobao_taobaoke_listurl_get($q,$outer_code){ //S8接口，可以自己拼装url
		$pid=$this->ApiConfig->taodianjin_pid;
		$url='http://s8.taobao.com/search?q='.rawurlencode($q).'&pid='.$pid.'&commend=all&unid='.$outer_code.'&taoke_type=1';
	    return $url;
	}
