...
class cron{
	...
	function __construct($duoduo){		
		...
		if ((isset ($_GET['admin']) && ddStrCode($_GET['admin'],DDKEY, 'DECODE') == 'admin_run') || ($_SESSION['ddadmin']['name']!='' && AJAX==0)) {  //判断是否是后台人工执行
			$this->admin_run=1;
			$this->admin_run_param='&admin='.ddStrCode('admin_run',DDKEY,'ENCODE');
		}
		else{
			$this->admin_run=0;
		...
		if($this->admin_run==0 && $next_time>date('Y-m-d H:i:s') && $this->cron_cache['jindu']==0){
			$this->_exit('时间未到，请稍后再试'.$next_time.'--'.date('Y-m-d H:i:s'));
		}
		
		if($this->log==1){
			create_file(DDROOT.'/data/cron/'.date('Ymd').'.txt',CUR_URL.'--'.date('Y-m-d H:i:s')."\r\n",FILE_APPEND);
		}
...
function cron_next_time($row){
	...
	elseif($row['fangshi']==2){
		if($row['last_time']!='2000-01-01'){  //如果上次执行时间不为空，用上次执行时间加设置的时钟
			$next_time=date('Y-m-d H:i:s',strtotime($row['last_time']." +".$row['interval_time']." minute"));
			if($next_time<=SJ){  //如果执行时间小于等于当前时间
				$next_time=1;  //可执行
			}
			else{
				$next_time=-1;  //不执行
	...
	if($next_time==1){
		$next_time=date('Y-m-d H:i:s',strtotime('-1 second'));
	}
	elseif($next_time==-1){
		$next_time=date('Y-m-d H:i:s',strtotime('+10 second'));
	}
	return $next_time;
}