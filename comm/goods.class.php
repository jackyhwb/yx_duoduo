...
class goods{
	...
	function show($pagesize,$page=1,$field='*',$where="1=1",$order_by){
		...
		$a=$this->duoduo->select_all($this->table_name,$field,$where.' and top<>2 order by top DESC,'.$order_by.' limit '.$frmnum.','.$pagesize);
		...
		if($this->quanju==1&&$page==1){
			$top_data=$this->duoduo->select_all($this->table_name,$field,'top=2 order by '.$order_by);
	...
	function do_item($item,$goods_attribute_arr=array(),$goods_type=array()){
		...
		if(FANLI==1){
			...
			$item['fanli']=(float)$item['discount_price']*(float)$item['fanli_bl']*0.01*$this->webset['fxbl'][3];
			$item['fanli_bl']=round($item['fanli_bl']*$this->webset['fxbl'][3],4);
		...
		if($item['tg_url']){
			$item['jump']=SITEURL.'/index.php?mod=jump&act=s8&url='.base64_encode($item['tg_url']);
	...
	function tuiguang($item){
		...
		$item['sina_url']="http://v.t.sina.com.cn/share/share.php?title=".urlencode($item['title'])."&url=".$tg_url;
		$item['qzone_url']="http://sns.qzone.qq.com/cgi-bin/qzshare/cgi_qzshare_onekey?url=".$tg_url."&title=".urlencode($item['title'])."&summary=".urlencode($item['title'])."&pics=".urlencode(tb_img($item['img'],220))."&desc=".urlencode($item['title']);
		$item['renren_url']="http://widget.renren.com/dialog/share?resourceUrl=".$tg_url."&srcUrl=".$tg_url."&title=".urlencode($item['title'])."&images=".urlencode($item['img'])."&description=".urlencode($item['title']);
		$item['qqim_url']='http://connect.qq.com/widget/shareqq/index.html?url='.$tg_url.'&desc='.urlencode($item['title']).'&pics='.urlencode($item['img']).'&site=bshare';
		$item['qqmb_url']="http://share.v.t.qq.com/index.php?c=share&a=index&title=".urlencode($item['title'])."&site=".$tg_url."&pic=".urlencode($item['img'])."&url=".$tg_url."&appkey=dcba10cb2d574a48a16f24c9b6af610c";
		return $item;
	}
