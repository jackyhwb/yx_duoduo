...
class duoduo {
	...
	function get_version($isformat = true) {
	    ...
	    $row = $this->fetch_array($rs,MYSQLI_NUM);
	    $mysql_version = $row[0];
	    $this->free_result($rs);
	    if ($isformat) {
	...
	function select($table, $sel_field, $where='1', $alert = 0) {
		...
		if($table_cache==1){
			$query=get_ddcache($sql,'sql/'.$this->table_cache_dir($table),$this->sql_cache_time);
			if($query!=false){
				return $query;
			}
		}
		...
		if($table_cache==1){
			set_ddcache($sql,$arr,'sql/'.$this->table_cache_dir($table));
		}
		return $arr;
	}
...
function set_constants($table) {
	...
	$query = $this->query($sql);
	$i = 1;
	$set = $this->fetch_array($query, MYSQLI_NUM);
	...
	public function do_report($row){ //淘宝获取订单处理办法
		...
		$row['item_title']=$this->real_escape_string($row['item_title']);
		$row['shop_title']=$this->real_escape_string($row['shop_title']);
    ...
    function trade_import($shuju) {
		...
		$status_key=5;//订单状态
		$pay_time_key=13;//结算时间
		$real_pay_fee_key=7;//实际成交额
		$commission_rate_key=6;//商品佣金比例
		$num_iid_key=17;//商品ID
		$seller_nick_key=12;//掌柜旺旺
		$shop_title_key=12;//所属店铺
		$commission_key=10;//商品佣金收益
		$commission_rate_butie_key=16;//补贴比率
		$commission_butie_key=14;//补贴金额
		$trade_id_former_key=18;//订单编号
		$taobao_uz_key=20;//订单编号
        foreach ((array)$shuju as $row) {
			...
			//$arr['item_title'] = mysql_real_escape_string($arr['item_title']);
           
			$arr['shop_title'] = preg_replace('/\'|"/','',$row[$shop_title_key]);
			//$arr['shop_title'] = mysql_real_escape_string($arr['shop_title']);
			 
			$arr['seller_nick'] = preg_replace('/\'|"/','',$row[$seller_nick_key]);
			//$arr['seller_nick'] = mysql_real_escape_string($arr['seller_nick']);
			...
			}elseif($arr['commission_rate']>0){
				$arr['commission'] =round($arr['commission_rate']*$arr['real_pay_fee'],2);
			}elseif($row[$commission_butie_key]>0){
    ...
    function trade_ruku($arr, $result) {
		$a = (int)$this->select('tradelist', 'id', 'mini_trade_id="' . $arr['mini_trade_id'] . '" and category_id>0 order by id desc');  //如果是api获取的，停止执行
		if($a>0){
			return false;
		}
			...
			if ($webset['ucenter']['open'] == 1) {
				...
				if ($ucid > 0) {
					...
					if (!$id) { // 不存在就插入多多
						...
						if ($uid <= 0) {
							echo '插入会员失败' . mysqli_error($this->link);
							exit;
						} 
	...
	function register($device='pc'){
		...
		if (isset ($_POST['username']) && $_POST['username'] != '') {
			...
			if ($uid <= 0) {
				exit (mysqli_error($this->link) . '插入会员失败');
			}
