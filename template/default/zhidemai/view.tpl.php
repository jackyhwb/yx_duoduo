          ...
          <div id="J_zdm_list">
              ...
              <?php if($row['shuxing_id']>0){?><i class='<?=$zhidemai['shuxing_class']?>'><?=$zhidemai['shuxing']?></i><?php }?>
                ...
                <div class="item-info J-item-info">
                  <div class="item-time"><?=date('m-d H:i:s',strtotime($zhidemai['starttime']))?> </div>
                ...
                <div class="J-expend-wrap-before item-vote clearfix">
                  ...
                  <div class="bshare-custom" style=" width:300px; margin-top:7px; float:left; margin-left:10px;">
                            <a title="分享到" href="http://www.bshare.cn/share" id="bshare-shareto" class="bshare-more" style="font-weight:normal; color:#555;">分享到:</a>
 								<a title="分享到QQ空间" class="bshare-qzone">QQ</a>
...
bShare.addEntry({
    title: "<?=$zhidemai['title']?>",
    url: "<?=SITEURL.'/index.php?mod=zhidemai&act=view&id='.$zhidemai['id']?>",
    summary: "<?=str_replace('"','',compact_html(strip_tags($zhidemai['content'])))?>",
    pic: "<?=preg_match('/^http/',$zhidemai['img'])?$zhidemai['img']:SITEURL.'/'.$zhidemai['img']?>"
});
</script>
                </div>             
