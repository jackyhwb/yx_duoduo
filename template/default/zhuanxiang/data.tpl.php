...
<?php foreach($ddgoods_list as $row){?>
						<li class="cell" id="<?=$row['id']?>&uid=<?=$dduser['id']?>" url="<?=u('tao','view',array('id'=>$row['id']))?>" youhui="<?=$row['discount_price']-$row['shouji_price']?>">
						...
						<div class="goods-txt-btn">
							...
							<p class="goods-info"><span class="sign red" title="返<?=TBMONEY?>">返<?=TBMONEY?></span></p>
