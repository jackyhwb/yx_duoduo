...
<?php }?>
    ...
    <table border="0" align="left" cellpadding="0" cellspacing="0" bordercolorlight="#9acd32" class="no_table">
		...
		<tr>
		  ...
		  <td style=" padding-left:20px"><?php if($dduser['mobile']!=''){?><div class="img-button"><p><input id="get_mobile_yzm" disabled type="button" value="获取验证码"/></p></div><?php }else{?><a style="color:#69F; text-decoration:underline" href="<?=u('user','info',array('do'=>'mobile','from'=>u(MOD,ACT,array('do'=>$do))))?>">添加/修改</a><?php }?></td>
		</tr>
        <tr>
		  <td height="35" style="text-align:right">短信验证码 *&nbsp;&nbsp;&nbsp;</td>
		  <td><input name="yzm" id="yzm" style="width:130px;" class="ddinput"/></td>
...
<?php }elseif($do=='tbnick'){?>
    ...
    <?php foreach($tbnick_arr as $k=>$v){?>
        <tr <?php if($v){?>id="tbnick_tr_<?=$k?>"<?php }?>>
		  <td height="35" style="text-align:right">淘宝账号或订单号 *&nbsp;&nbsp;&nbsp;</td>
