...
if($domain!=''){//判断是否是网址
	...
	foreach($chong_mall_url as $vo){
		if($vo['type']==1){
			if(strpos($url,$vo['host'])!==false){
				$mid=(int)$duoduo->select(get_mall_table_name(),'id','url like "%'.$vo['host'].'%" and is_search=0 ');
			}
		}elseif($vo['type']==0){
	...
	if(empty($mid)){
		$mid=(int)$duoduo->select(get_mall_table_name(),'id','domain="'.$domain.'" and is_search=0 ');
	}
	...
	$mid = $duoduo->select(get_mall_table_name(),'id','title like "%'.$q.'%" and is_search=0 ');
	if($mid > 0){//判断是否网站是否添加了该商城
		$t=1;
