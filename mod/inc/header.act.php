...
if($_COOKIE['userlogininfo']!='' && (int)$userlogininfo['ver']==0 && ACT!='exit'){
	jump(SITEURL.'/?mod=user&act=exit&domain_ignore=1&from='.urlencode(CURURL));
}
...
if($hcookieuid>0 && $hcookiepassword<>NULL){	
	...
	if($user['id']>0){
		...
		//if(preg_match('/qq\.com$/',$dduser['email']))
