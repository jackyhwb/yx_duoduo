...
<table cellspacing="0" width="100%" style="border:1px  solid #DCEAF7; border-bottom:0px; background:#E9F2FB">
        <tr>
              <td width="500" class="bigtext">&nbsp;<img src="images/arrow.gif" width="16" height="22" align="absmiddle" />&nbsp;<a href="<?=u(MOD,'iframe',array('a'=>'add','code'=>$code))?>" class="link3">[添加爆料]</a> <a href="<?=u(MOD,'iframe',array('a'=>'index','type'=>'me','code'=>$code))?>" class="link3">[我的爆料]</a>  <a href="<?=DD_U_URL?>/index.php?g=alliance&m=zhidemai&a=lists&lanmu=5&type=yun&url=<?=urlencode(SITEURL)?>" target="_blank" class="link3">[采集爆料]</a><?php if($reycle==1){?><a href="<?=u(MOD,ACT)?>" class="link3">[返回列表]</a><?php }else{?><a href="<?=u(MOD,ACT,array('reycle'=>1))?>" class="link3">[回收站]</a><?php }?> <a href="<?=u(MOD,ACT,array('code'=>$code,'guoqi'=>1))?>" class="link3" onclick="return confirm('确认要删除值得买到期数据？');">[删除到期数据]</a></td>
              <td width=""></td>
      ...
      <table id="listtable" border=1 cellpadding=0 cellspacing=0 bordercolor="#dddddd">
	    ...
	    <tr>
          ...
          <td><span class="ddnowrap" style="width:130px" title="<?=$r["subtitle"]?>"><?=$r["subtitle"]?></span></td>
