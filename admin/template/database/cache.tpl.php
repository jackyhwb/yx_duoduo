...
<table id="addeditable" border=1 cellpadding=0 cellspacing=0 bordercolor="#dddddd">
  ...
  <tr>
    ...
    <td>&nbsp;<input style="width:30px" name="paipai[cache_time]" value="<?=$webset['paipai']['cache_time']?>" />&nbsp;<span class="zhushi">单位(小时)，设为为0即为不缓存，目录为data/temp/paipai。</span><input type="button" value="删除缓存" onclick="javascript:openpic('../<?=u('cron','run',array('jiaoben'=>'cron_delpaipaicache'))?>','upload','450','350')" /></td>
  </tr>
  <tr>
    <td align="right">亿起发缓存时间：</td>
    <td>&nbsp;<input style="width:30px" name="yiqifaapi[cache_time]" value="<?=$webset['yiqifaapi']['cache_time']?>" />&nbsp;<span class="zhushi">单位(小时)，设为为0即为不缓存，目录为data/temp/yiqifaapi。</span><input type="button" value="删除缓存" onclick="javascript:openpic('../<?=u('cron','run',array('jiaoben'=>'cron_delyiqifacache'))?>','upload','450','350')" /></td>
  </tr>
  <tr>
    <td align="right">59秒缓存时间：</td>
    <td>&nbsp;<input style="width:30px" name="wujiumiaoapi[cache_time]" value="<?=$webset['wujiumiaoapi']['cache_time']?>" />&nbsp;<span class="zhushi">单位(小时)，设为为0即为不缓存，目录为data/temp/wujiumiaoapi。</span><input type="button" value="删除缓存" onclick="javascript:openpic('../<?=u('cron','run',array('jiaoben'=>'cron_del59miaocache'))?>','upload','450','350')" /></td>
  </tr>
  <tr>
